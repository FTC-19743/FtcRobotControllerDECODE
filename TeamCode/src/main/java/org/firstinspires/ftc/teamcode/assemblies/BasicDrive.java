...
//
    ...
    public void waitForRobotToStop(long timeout){
        ...
        double xVelo = localizer.velX_mmS;
        double yVelo = localizer.velY_mmS;
        double hVelo = localizer.velHeading_radS;
        ...
        while((Math.abs(xVelo)>VELO_THRESHOLD || Math.abs(yVelo)>VELO_THRESHOLD/*||hVelo>ANGLE_VELO_THRESHOLD*/) && teamUtil.keepGoing(timeOutTime)){
            teamUtil.pause(10);
            if(details) teamUtil.log(String.format("xPos: %.0f xVel: %.2f yPos: %.0f yVel: %.2f hVel: %.2f", localizer.posX_mm, xVelo, localizer.posY_mm, yVelo , hVelo));
            loop();
            xVelo = localizer.velX_mmS;
            yVelo = localizer.velY_mmS;
            hVelo = localizer.velHeading_radS;
        }
    ...
    public double getRawHeadingODO() {
        ...
        return Math.toDegrees(localizer.heading_rad);
    }
...
//deceleration
    ...
    public double getPosX() {
        loop();
        double pos = localizer.posX_mm;
        while (Double.isNaN(pos)) {
            teamUtil.log("ERROR ----------------------------------------------- Octoquad returned NaN!");
            loop();
            pos = localizer.posX_mm;
        }
        return pos;
    }
    ...
    public double getPosY() {
        loop();
        double pos = localizer.posY_mm;
        while (Double.isNaN(pos)) {
            teamUtil.log("ERROR ----------------------------------------------- Octoquad returned NaN!");
            loop();
            pos = localizer.posY_mm;
        }
        return pos;
    }
    ...
    public boolean strafeToTarget(double driveHeading, double robotHeading, double velocity, double strafeTarget, long timeout) {
        ...
        long timeOutTime = System.currentTimeMillis() + timeout;
        loop();
        teamUtil.log("strafeToTarget: Current: " + localizer.posY_mm + " Target: " + strafeTarget);
        if (driveHeading<180) {
            while (localizer.posY_mm < strafeTarget && teamUtil.keepGoing(timeOutTime)) {
                loop();
        ...
        if (System.currentTimeMillis() > timeOutTime) {
            ...
            teamUtil.log("strafeToTarget - FINISHED : Current: " + localizer.posY_mm);
            return true;
        }
    }
    ...
    public boolean straightHoldingStrafeEncoder(double maxVelocity, double straightTarget, double strafeTarget, int robotHeading, double endVelocity, boolean powerBraking, ActionCallback action, double actionTarget, long timeout) {
        ...
        loop();
        if (details) teamUtil.log("Starting Forward Pos: "+ localizer.posX_mm);
        if (details) teamUtil.log("Starting Strafe Pos: "+ localizer.posY_mm);
        // if (details) teamUtil.log("Starting Forward Encoder: "+ forwardEncoder.getCurrentPosition());
        //if (details) teamUtil.log("Starting Strafe Encoder: "+ strafeEncoder.getCurrentPosition());
        ...
        double velocityChangeNeededDecel;
        double driveHeading;
        double startEncoder = localizer.posX_mm;
        //double startEncoder = forwardEncoder.getCurrentPosition();
        boolean goingUp;
        ...
        distanceRemaining = Math.abs(straightTarget - localizer.posX_mm);
        ...
        while ((distanceRemaining > (totalTics-accelerationDistance))&&teamUtil.keepGoing(timeoutTime)) {
            ...
            adjustedDriveHeading = driveHeading + MathUtils.clamp((localizer.posY_mm - strafeTarget)* STRAIGHT_HEADING_DECLINATION, -STRAIGHT_MAX_DECLINATION, STRAIGHT_MAX_DECLINATION) * headingFactor;
        ...
        if (!powerBraking) {  //-------Normal Deceleration Phase
            ...
            currentVelocity = Math.abs(localizer.velX_mmS); // find the actual current velocity of the robot
            if (endVelocity > MIN_END_VELOCITY) {  // Assume that end position does not need to be precise and let the robot drift to a stop (or crash into something...)
                ...
                stopMotors();
                while ((distanceRemaining > 0) && Math.abs(localizer.velX_mmS)>endVelocity && teamUtil.keepGoing(timeoutTime)) {  // wait for distance or speed to achieve goal
                    loop();
                    currentPos = localizer.posX_mm;
                    if (Double.isNaN(currentPos)) {
                        teamUtil.log("ERROR ----------------------------------------------- Pinpoint returned NaN during power braking");
                    ...
                    distanceRemaining = (!goingUp) ? currentPos-straightTarget : straightTarget - currentPos;
                    if (details) teamUtil.log("Power Braking: Velocity: "+ localizer.velX_mmS + " MMs Remaining: " + distanceRemaining);
                    if(action!=null&&!actionDone&&((goingUp&&currentPos>=actionTarget)||(!goingUp&&currentPos<=actionTarget))){
                        action.action();
                        actionDone=true;
                    }
        ...
        setBulkReadOff();
        lastVelocity = Math.max(endVelocity,MIN_START_VELOCITY);
        teamUtil.log("driveStraightToTargetWithStrafeEncoderValue--Finished.  Current Forward Pos:" + localizer.posX_mm);
        return true;
    }
    ...
    public boolean strafeHoldingStraightEncoder(double maxVelocity, double strafeTarget, double straightTarget, int robotHeading, double endVelocity, ActionCallback action, double actionTarget, long timeout) {
        ...
        long timeoutTime = startTime+timeout;
        loop();
        if (details) teamUtil.log("Starting Forward Pos: "+ localizer.posY_mm);
        if (details) teamUtil.log("Starting Strafe Pos: "+ localizer.posX_mm);
        ...
        double startEncoder = localizer.posY_mm;
        boolean goingUp;
        ...
        distanceRemaining = Math.abs(strafeTarget - localizer.posY_mm);
        ...
        while ((distanceRemaining > (totalTics-accelerationDistance))&&teamUtil.keepGoing(timeoutTime)) {
            ...
            adjustedDriveHeading = driveHeading + MathUtils.clamp((localizer.posX_mm - straightTarget)* STRAFE_HEADING_DECLINATION, -STRAFE_MAX_DECLINATION, STRAFE_MAX_DECLINATION) * headingFactor;
            if (details) {
                teamUtil.log("dh: " + adjustedDriveHeading);
            }
        ...
        while ((distanceRemaining > 0)&&teamUtil.keepGoing(timeoutTime)) {
            ...
            distanceRemaining = (!goingUp) ? currentPos-strafeTarget : strafeTarget - currentPos;
            currentVelocity = MAX_STRAIGHT_DECELERATION * distanceRemaining + endVelocity;
            adjustedDriveHeading = driveHeading + MathUtils.clamp((localizer.posX_mm - straightTarget)* STRAFE_HEADING_DECLINATION, -STRAFE_MAX_DECLINATION, STRAFE_MAX_DECLINATION) * headingFactor;            if (details) {
                teamUtil.log("dh: " + adjustedDriveHeading);
            }
        ...
        teamUtil.log("strafeHoldingStraightEncoder--Finished.  Current Strafe Encoder:" + localizer.posY_mm);
        return true;
    }
    ...
    public boolean straightHoldingStrafePower(float power, double straightTarget, double strafeTarget, int robotHeading, ActionCallback action, double actionTarget, long timeout) {
        ...
        double totalTics = Math.abs(startEncoder-straightTarget);
        teamUtil.log("straightHoldingStrafePower target: " + straightTarget +  " Strafe target: " + strafeTarget + " robotH: " + robotHeading + " Power: " + power + " TotalMMss: " + totalTics + " Starting Forward Pos: "+ localizer.posX_mm + " Starting Strafe Pos: "+ localizer.posY_mm + " Starting Heading:" + getHeadingODO());
        double distanceRemaining = Math.abs(straightTarget - localizer.posX_mm);
        setMotorsWithEncoder();
        boolean actionDone = false;
        double currentPos;
        double adjustedDriveHeading;
        ...
        teamUtil.log("straightHoldingStrafePower--Finished.  Current Forward Pos:" + localizer.posX_mm);
        return true;
    }
    ...
    public boolean moveToX (float power, int x, int driveHeading, int robotHeading, ActionCallback action, double actionTarget, int timeout) {
        ...
        teamUtil.log("moveToX target: " + x + " driveH: " + driveHeading + " robotH: " + robotHeading + " Power: " + power + " TotalMMss: " + totalTics + " Starting Forward Pos: "+ localizer.posX_mm + " Starting Strafe Pos: "+ localizer.posY_mm + " Starting Heading:" + getHeadingODO());
        double distanceRemaining = Math.abs(x - localizer.posX_mm);
        setMotorsWithEncoder();
        boolean actionDone = false;
        double currentPos;
        while ((distanceRemaining > 0) && teamUtil.keepGoing(timeoutTime)) {
        ...
        teamUtil.log("moveToX--Finished.  Current Forward Pos:" + localizer.posX_mm);
        return true;
    }
    ...
    public boolean moveToY (float power, int y, int driveHeading, int robotHeading, ActionCallback action, double actionTarget, int timeout) {
        ...
        teamUtil.log("moveToY target: " + y + " driveH: " + driveHeading + " robotH: " + robotHeading + " Power: " + power + " TotalMMss: " + totalTics + " Starting Forward Pos: "+ localizer.posX_mm + " Starting Strafe Pos: "+ localizer.posY_mm + " Starting Heading:" + getHeadingODO());
        double distanceRemaining = Math.abs(y - localizer.posY_mm);
        setMotorsWithEncoder();
        boolean actionDone = false;
        double currentPos;
        while ((distanceRemaining > 0) && teamUtil.keepGoing(timeoutTime)) {
        ...
        teamUtil.log("moveToY--Finished.  Current Forward Pos:" + localizer.posY_mm);
        return true;
    }
    ...
    public boolean strafeHoldingStraightPower(float power, double strafeTarget, double straightTarget, int robotHeading, ActionCallback action, double actionTarget, long timeout) {
        ...
        double totalTics = Math.abs(startEncoder-strafeTarget);
        teamUtil.log("strafeHoldingStraightPower target: " + strafeTarget +  " Straight target: " + straightTarget + " robotH: " + robotHeading + " Power: " + power + " TotalMMss: " + totalTics + " Starting Forward Pos: "+ localizer.posX_mm + " Starting Strafe Pos: "+ localizer.posY_mm + " Starting Heading:" + getHeadingODO());
        loop();
        double distanceRemaining = Math.abs(strafeTarget - localizer.posY_mm);
        setMotorsWithEncoder();
        boolean actionDone = false;
        double currentPos;
        double adjustedDriveHeading;
        ...
        teamUtil.log("strafeHoldingStraightPower--Finished.  Current Strafe Encoder:" + localizer.posY_mm);
        return true;
    }
    ...
    public boolean moveToXHoldingStrafe(float power, double xTarget, double yTarget, int driveHeading, int robotHeading, float endPower, ActionCallback action, double actionTarget, long timeout) {
        ...
        teamUtil.log("moveToXHoldingStrafe xTarget: " + xTarget +  " yTarget: " + yTarget + " robotH: " + robotHeading + " driveH: " + driveHeading + " Power: " + power + " TotalMMss: " + totalTics + " Starting Forward Pos: "+ localizer.posX_mm + " Starting Strafe Pos: "+ localizer.posY_mm + " Starting Heading:" + getHeadingODO());
        double distanceRemaining = Math.abs(xTarget - localizer.posX_mm);
        setMotorsWithEncoder();
        boolean actionDone = false;
        double currentPos;
        double adjustedDriveHeading;
        ...
        while ((distanceRemaining > 0) && teamUtil.keepGoing(timeoutTime)) {
            ...
            adjustedDriveHeading = driveHeading - MathUtils.clamp(distanceToLine(xTarget, yTarget, driveHeading, localizer.posX_mm, localizer.posY_mm)* STRAIGHT_HEADING_DECLINATION, -STRAIGHT_MAX_DECLINATION, STRAIGHT_MAX_DECLINATION);
        ...
        teamUtil.log("moveToXHoldingStrafe--Finished.  Current xPos:" + localizer.posX_mm + " Current yPos: "+ localizer.posY_mm);
        return true;
    }
    public boolean moveToYHoldingLine(float power, double yTarget, double xTarget, int driveHeading, int robotHeading, float endPower, ActionCallback action, double actionTarget, long timeout) {
        ...
        teamUtil.log("moveToYHoldingLine yTarget: " + yTarget +  " xTarget: " + xTarget + " robotH: " + robotHeading + " driveH: " + driveHeading + " Power: " + power + " TotalMMss: " + totalTics + " Starting Forward Pos: "+ localizer.posX_mm + " Starting Strafe Pos: "+ localizer.posY_mm + " Starting Heading:" + getHeadingODO());
        double distanceRemaining = Math.abs(yTarget - localizer.posY_mm);
        setMotorsWithEncoder();
        boolean actionDone = false;
        double currentPos;
        double adjustedDriveHeading;
        ...
        teamUtil.log("moveToYHoldingLine--Finished.  Current yPos:" + localizer.posY_mm + " Current xPos: "+ localizer.posX_mm);
        return true;
    }
    ...
    public void moveToV2(double maxVelocity, double strafeTarget, double straightTarget, double robotHeading, double endVelocity,ActionCallback action, double actionTarget, boolean endInDeadband, long timeout){
        ...
        boolean strafeIncreasing = (strafeTarget - localizer.posY_mm > 0);
        boolean straightIncreasing = (straightTarget - localizer.posX_mm > 0);
        if(endVelocity<MIN_END_VELOCITY){
            endVelocity=MIN_END_VELOCITY;
            lastVelocity=0;
        }
        ...
        while(!withinThreshold&&teamUtil.keepGoing(timeoutTime)) {
            loop();
            double straightChange = straightTarget - localizer.posX_mm;
            double strafeChange = strafeTarget - localizer.posY_mm;
            ...
            if(endInDeadband){ // We must be at or near the target
                ...
                strafeTargetAchieved = strafeTargetAchieved || strafeIncreasing ? localizer.posY_mm > strafeTarget - MOVE_TO_THRESHOLD : localizer.posY_mm < strafeTarget + MOVE_TO_THRESHOLD;
                straightTargetAchieved = straightTargetAchieved || straightIncreasing ? localizer.posX_mm > straightTarget - MOVE_TO_THRESHOLD : localizer.posX_mm < straightTarget + MOVE_TO_THRESHOLD;
    ...
    public void moveTo(double maxVelocity, double strafeTarget, double straightTarget, double robotHeading, double endVelocity,ActionCallback action, double actionTarget, boolean endInDeadband, long timeout){
        ...
        teamUtil.log("MoveTo FINISHED Heading: " + Math.toDegrees(localizer.heading_rad) + " Xpos: " + localizer.posX_mm + " Ypos: " + localizer.posY_mm);
    }
    ...
    public void moveToPower(float maxPower, double strafeTarget, double straightTarget, double robotHeading, float  endPower,ActionCallback action, double actionTarget, boolean endInDeadband, long timeout){
        ...
        teamUtil.log("MoveToPower FINISHED Heading: " + Math.toDegrees(localizer.heading_rad) + " Xpos: " + localizer.posX_mm + " Ypos: " + localizer.posY_mm);
    }
